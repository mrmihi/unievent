...
const VViewVenueProfile = () => {
    ...
    useEffect(() => {
        // Fetch venue reviews
        axios.get(`http://localhost:5000/api/reviews/venue/${id}`).then((res) => {
            console.log(res.data);
            setReviews(res.data);
    ...
    useEffect(() => {
        // get all venues
        axios.get('http://localhost:5000/api/venues').then((res) => {
            const random = res.data.sort(() => Math.random() - Math.random()).slice(0, 3);
                ...
                <Grid item xs={4}>
                    <div className='flex flex-col justify-between h-full'>
                        <div className='mt-20'>
                            ...
                            <Rating name="rating" value={averageRating} precision={0.5} readOnly />
                        </div>
                        ...
                        {activeTab === 'ratings' && (
                            <Box className="mb-10 mt-10">
                                {reviews.map((data, index) => (
                                    <ReviewCard key={90} user={data.organizer.name} rating={data.rating} description={data.review} />
                                ))}
                            </Box>
                        )}
                                            ...
                                            {randomVenues.map((venue, index) => (
                                                <Grid key={index} item xs={12} sm={6} md={4}>
                                                    <Card sx={{ height: '100%' }}>
                                                        ...
                                                        />
                                                        <CardContent>
                                                            <Typography gutterBottom variant="h5" component="div">Name: {venue.name}</Typography>
                                                            <Typography variant="body2" color="text.secondary">Location: {venue.location}</Typography>
                                                            <Typography variant="body2" color="text.secondary">Price: {venue.price}</Typography>
                                                        </CardContent>
                                                    </Card>
                                                </Grid>
                                            ))}
